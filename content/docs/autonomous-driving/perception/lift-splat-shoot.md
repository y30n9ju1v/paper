...
## 💡 한 줄 요약
임의의 카메라 리그로부터 멀티뷰 이미지를 받아, 각 픽셀의 깊이에 대한 확률 분포를 예측하여 3D 공간에 "Lift"하고 BEV 그리드에 "Splat"한 뒤, 이 BEV cost map에 궤적을 "Shoot"하여 모션 플래닝까지 end-to-end로 수행하는 아키텍처를 제안한다.

## 📌 개요 및 핵심 기여 (Key Contributions)
- **저자**: Jonah Philion, Sanja Fidler (NVIDIA, University of Toronto, Vector Institute)
- **발행년도**: 2020 (arXiv:2008.05711)
- **주요 기여점**:
  1. LiDAR 없이 멀티뷰 카메라 이미지만으로 ego-car 기준 BEV 표현을 직접 학습하는 end-to-end 미분 가능 아키텍처 "Lift-Splat-Shoot" 제안
  2. 각 픽셀에 대해 깊이의 카테고리 분포를 예측하여 frustum 형태의 3D 포인트 클라우드를 생성(Lift)하고, Pillar 기반 sum pooling으로 BEV 텐서를 구성(Splat)하는 방법 설계
  3. Cumulative Sum Trick으로 패딩 없이 메모리 효율적인 sum pooling을 구현하여 역전파 속도를 2배 향상
  4. BEV cost map에 template trajectory를 쏘아(Shoot) 최적 경로를 선택하는 end-to-end 모션 플래닝까지 통합
  5. 객체 분할, 지도 분할, 모션 플래닝 태스크에서 기존 방법(CNN, OFT)을 능가하고, 카메라 캘리브레이션 오류·카메라 누락에도 강건함을 입증

## 🎯 관련 연구 흐름 및 기존 한계 (Related Work & Motivation)
- **연구 흐름**: 단안 이미지 평면에서 2D 탐지 후 3D bounding box를 회귀하는 방법에서, 깊이 예측과 BEV 탐지를 분리 학습하는 pseudolidar 방식으로, 그리고 고정 voxel을 이미지에 투영해 feature를 모으는 OFT(Orthographic Feature Transform) 방식으로 발전해왔습니다. 이후 MonoLayout, Pyramid Occupancy Networks(PON), FISHING Net 등 카메라 파라미터를 이용해 이미지를 BEV로 직접 변환하는 연구가 이어졌습니다.
- **기존 한계점**:
  1. 기존 컴퓨터 비전 모델은 입력 이미지와 동일한 좌표계 내에서 예측을 출력하도록 설계되어, 자율주행에 필요한 ego-car 기준 BEV 좌표계 출력에 바로 적용하기 어려움
  2. 개별 카메라에 단일 이미지 탐지기를 따로 적용한 뒤 ego 프레임으로 변환하는 방식은 카메라 간 최적 융합 전략을 데이터 기반으로 학습하거나 downstream planner의 피드백으로 역전파할 수 없음
  3. OFT 같은 기존 방법은 한 픽셀의 feature를 깊이와 무관하게 모든 voxel에 동일하게 기여시켜, 실제 깊이 분포(불확실성)를 반영하지 못함
- **이 논문의 접근 방식**: 각 픽셀마다 깊이에 대한 확률 분포(categorical distribution)를 예측하고, 이를 이용해 frustum 형태의 3D 피처 포인트 클라우드를 생성(Lift)한 뒤, BEV 평면에 통합(Splat)하여 하나의 통합된 BEV CNN으로 처리합니다. 전체 파이프라인이 end-to-end로 미분 가능하며, 카메라 캘리브레이션에 대한 강건성도 데이터로부터 학습합니다.
...
- **사용 데이터셋 / 벤치마크**: nuScenes(6카메라, 1K 장면, 20초), Lyft Level 5(6카메라). 태스크는 객체 분할(Car, Vehicle), 지도 분할(Drivable Area, Lane Boundary), 모션 플래닝.
...
Lift-Splat-Shoot은 임의의 카메라 리그로부터 LiDAR 없이 BEV 표현을 end-to-end로 학습하는 아키텍처입니다. 깊이 불확실성을 카테고리 분포로 모델링하는 핵심 아이디어 덕분에, 단순한 기준선보다 우수한 성능을 보이며 캘리브레이션 오류와 카메라 누락에도 강건합니다.

- **LiDAR 없는 자율주행 인식**: BEVFusion, BEVDet, BEVDepth 등 이후 BEV 인식 연구의 토대가 된 seminal 논문. 카메라만으로 BEV를 추론하는 패러다임을 확립했습니다.
- **Camera rig 확장성**: 카메라 수와 위치가 바뀌어도 재학습 없이 zero-shot 전이가 가능합니다. 실제 자율주행 차량 개발에서 카메라 배치 변경 시 유용합니다.
- **End-to-end 모션 플래닝**: 인식과 플래닝을 통합 학습하여, 플래너의 피드백이 인식 모듈 학습에 반영됩니다.
- **한계점 및 아쉬운 점**: 야간이나 장거리에서 LiDAR 대비 성능 격차가 존재하며, 단일 타임스텝만 처리하므로 temporal 정보를 전혀 활용하지 못합니다(이는 이후 BEVDet4D, BEVFormer 등이 해결). 모션 플래닝 성능도 LiDAR 기반보다 명확히 낮아, 실제 배포에는 추가 검증이 필요합니다.
