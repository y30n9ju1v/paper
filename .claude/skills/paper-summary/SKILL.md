...
- 논문 포스트 작성/수정이 끝나면 `content/docs/glossary/`의 해당 카테고리 개념정리도 함께 업데이트합니다 (아래 "개념정리 유지보수" 참고).

## 개념정리 유지보수 (Glossary Maintenance)

논문을 새로 추가하거나 기존 논문을 업데이트할 때마다, 그 논문의 **핵심 개념** 섹션에 나온 개념 중 아직 개념정리에 없는 것을 해당 카테고리의 개념정리 파일에 추가합니다.
...
- 카테고리 → 개념정리 파일 매핑:
  ...
  - `autonomous-driving/dataset/` 등 아직 전용 개념정리가 없는 카테고리는, 논문이 3개 이상 쌓이고 개념정리가 필요해 보이면 사용자에게 새 개념정리 파일을 만들지 물어봅니다.
- 추가 절차:
  1. 새 논문의 **핵심 개념** 항목들을 훑어보고, 해당 개념정리 파일에 이미 있는 개념(같은 개념의 다른 표기 포함)은 건너뜁니다.
  2. 새로운 개념은 기존 개념정리의 주제별 섹션(`## BEV(Bird's-Eye-View) 공통 개념` 등) 중 가장 잘 맞는 곳에 추가합니다. 맞는 섹션이 없으면 새 `##` 섹션을 만듭니다.
  3. 정의는 그 논문 파일에 이미 쓴 문장을 재사용하되, 다른 논문에서도 통용되는 일반적인 설명이 되도록 다듬습니다 (특정 논문의 실험 수치보다는 개념 자체의 정의에 집중).
  4. **기초 개념도 함께 점검합니다.** 위 2~3단계로 전문 용어를 다 추가한 뒤, "이 논문을 이해하려면 알아야 하는데 전문 용어가 아니라서 놓치기 쉬운 사전지식"이 있는지 반드시 확인합니다 — 예: 새 평가 지표가 상관계수(PLCC/SRCC 등)를 쓴다면 "상관계수가 뭔지"부터, 새 방법이 어떤 통계·수학 개념(베이지안, 엔트로피 등)을 전제한다면 그 개념 자체부터 짚어야 할 수 있습니다. 해당되는 게 있으면 개념정리 최상단 "## 기초 개념 (사전지식)" 섹션에 추가합니다. 이미 있는 사전지식과 겹치면 건너뜁니다.
  5. 개념정리 안의 각 논문 링크 목록(파일 상단 소개 문장)에 새 논문 링크도 추가합니다.
  6. 다른 개념정리 파일이나 논문 포스트를 향한 링크(`/posts/papers/<slug>/`)를 추가했다면, 그 slug의 파일이 실제로 존재하는지 확인합니다.
