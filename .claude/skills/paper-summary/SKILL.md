...
- 추가 절차:
  ...
  4. **기초 개념도 함께 점검합니다.** 위 2~3단계로 전문 용어를 다 추가한 뒤, "이 논문을 이해하려면 알아야 하는데 전문 용어가 아니라서 놓치기 쉬운 사전지식"이 있는지 반드시 확인합니다 — 예: 새 평가 지표가 상관계수(PLCC/SRCC 등)를 쓴다면 "상관계수가 뭔지"부터, 새 방법이 어떤 통계·수학 개념(베이지안, 엔트로피 등)을 전제한다면 그 개념 자체부터 짚어야 할 수 있습니다. 해당되는 게 있으면 용어집 최상단 "## 기초 개념 (사전지식)" 섹션에 추가합니다. 이미 있는 사전지식과 겹치면 건너뜁니다.
