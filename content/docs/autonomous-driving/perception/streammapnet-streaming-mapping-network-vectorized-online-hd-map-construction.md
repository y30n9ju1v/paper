...
## 💡 한 줄 요약
StreamMapNet은 Multi-Point Attention과 스트리밍 시간 융합으로 넓은 인식 범위(100×50m)에서도 안정적인 벡터 HD 맵을 온라인으로 구성하며, 기존 벤치마크의 데이터 누출 문제를 발견해 공정한 새 split을 제안한다.

## 📌 개요 및 핵심 기여 (Key Contributions)
- **저자**: Tianyuan Yuan, Yicheng Liu, Yue Wang, Yilun Wang, Hang Zhao (Tsinghua University, University of Southern California)
- **발행년도**: 2023/2024 (arXiv 2308.12570)
- **주요 기여점**:
  1. Multi-Point Attention — 폴리라인 전체 $N_p$개 점을 참조점으로 사용해 넓은 범위의 비국소(non-local) 어텐션을 효율적으로 구현
  2. Streaming 시간 융합 — Query Propagation과 BEV Fusion(GRU)으로 프레임 수와 무관한 상수 비용으로 장기 이력을 통합
  3. 지리적 겹침 없는 공정한 새 데이터 split 제안 — 기존 nuScenes/Argoverse2 split의 데이터 누출 문제를 규명

## 🎯 관련 연구 흐름 및 기존 한계 (Related Work & Motivation)
- **연구 흐름**: 온라인 벡터 HD 맵 구성은 HDMapNet(래스터 세그 + 후처리) → VectorMapNet(DETR 기반 end-to-end) → MapTR(계층적 쿼리) → BeMapNet(Bézier 곡선 기반)으로 발전해왔다. 한편 카메라 기반 3D 탐지의 시간 모델링은 BEVDet4D, BEVFormer v2 같은 stacking 전략에서 VideoBEV, StreamPETR, Sparse4D v2 같은 streaming 전략으로 이동했다. StreamMapNet은 이 streaming 아이디어를 HD 맵 구성에 최초로 적용한다.
- **기존 한계점**:
  1. 단일 프레임 입력 — VectorMapNet, MapTR 등은 단일 프레임만 사용해 폐색·역광 등 어려운 환경에서 오류가 발생하고 프레임 간 시간적 비일관성 문제가 생긴다.
  2. 좁은 인식 범위 — 기존 방법은 60×30m 범위에선 잘 작동하나 100×50m로 넓히면 성능이 급락한다. 표준 Deformable Attention이 기다란 맵 요소의 비국소 형태를 포착하지 못하기 때문이다.
  3. 벤치마크 공정성 문제 — nuScenes 공식 split에서 검증 위치의 85%가 학습 데이터와 겹치고, Argoverse2도 54% 겹쳐 모델이 위치를 암기해 과적합되고 실제 일반화 능력을 측정할 수 없다.
...
## 🛠️ Chapter 3: StreamMapNet Model
...
**핵심 개념**
- **FPN (Feature Pyramid Network)**: 다양한 해상도의 특징 맵을 계층적으로 합쳐 크고 작은 객체를 모두 잘 탐지하는 멀티스케일 피처 구조
...
**수식 예제 (Standard Deformable DETR, i번째 레이어)**
...
**수식 예제 (Multi-Point Attention, StreamMapNet)**
...
**Query Propagation (희소 쿼리 레벨)**: 맵 요소(차선, 경계 등)는 프레임 간에 거의 변하지 않습니다. 따라서 이전 프레임에서 높은 신뢰도로 예측된 상위 $k$개의 쿼리를 다음 프레임으로 전달(propagate)합니다. 이 쿼리들은 자차 좌표계가 달라졌으므로 변환 행렬 $\boldsymbol{T}$로 좌표계를 맞춥니다.
...
**BEV Fusion (밀집 BEV 피처 레벨)**: 희소 쿼리 전파와 달리, 밀집 BEV 피처도 이전 프레임 정보를 담을 수 있습니다. 이전 프레임의 BEV 피처를 자차 이동에 맞게 워핑(warp)한 뒤, GRU로 현재 BEV 피처와 융합합니다.
...
- **사용 데이터셋 / 벤치마크**: nuScenes(기존 공식 split 및 새 geographic split), Argoverse2(기존 split 및 새 geographic split, 0% 겹침).
- **주요 성과**: Argoverse2 새 Split 100m 범위에서 MapTR 대비 +11.0 mAP, nuScenes 원본 Split에서 SOTA 62.9 mAP(BeMapNet 대비 +3.1), 14.2 FPS의 실시간 추론 속도 유지. 기존 nuScenes/Argoverse2 split은 각각 85%, 54%의 학습-검증 위치 겹침이 있어 새 split 도입 시 모든 방법의 성능이 약 50% 하락.
...
- **한계점 및 아쉬운 점**: 벤치마크 재정의라는 기여가 모델 자체의 기여를 가릴 만큼 크며, 새 split에서의 절대 성능은 여전히 낮은 편(100m에서 mAP 51.2)이라 실제 배포까지는 추가 개선이 필요하다. 또한 LiDAR 없이 카메라만으로 먼 거리의 정밀도를 확보하는 데는 한계가 있다.
