...
## 💡 한 줄 요약
멀티카메라 RGB 이미지만으로 2D-3D 공간 어텐션과 Poisson Reconstruction 기반 밀집 GT 생성 파이프라인을 통해, 가려진 영역까지 포함한 밀집(dense) 3D 시맨틱 점유(occupancy)를 예측하는 SurroundOcc를 제안한다.

## 📌 개요 및 핵심 기여 (Key Contributions)
- **저자**: Yi Wei, Linqing Zhao, Wenzhao Zheng, Zheng Zhu, Jie Zhou, Jiwen Lu (Tsinghua University, Tianjin University, PhiGent Robotics)
- **발행년도**: 2023 (arXiv 2303.09551v2)
- **주요 기여점**:
  1. BEV 대신 3D 볼륨 쿼리를 구성하고 2D-3D 공간 어텐션으로 멀티카메라 특징을 집계하는 방법 제안
  2. 멀티스케일 3D U-Net과 decayed loss로 저해상도부터 고해상도까지 점진적으로 정제하는 occupancy 예측 구조 설계
  3. 멀티프레임 LiDAR 스티칭 + Poisson Surface Reconstruction으로, 별도 비용 없이 희소 LiDAR로부터 밀집 3D occupancy GT를 자동 생성하는 파이프라인 제시
  4. nuScenes, SemanticKITTI(제로샷) 벤치마크에서 기존 방법 대비 큰 폭의 성능 향상 달성

## 🎯 관련 연구 흐름 및 기존 한계 (Related Work & Motivation)
- **연구 흐름**: 자율주행 3D 인식은 (1) bounding box 기반 3D 객체 검출 → (2) depth map 기반 장면 재구성 → (3) BEV(Bird's Eye View) 기반 인식(BEVFormer 등)의 순서로 발전해왔습니다. 실내 환경에서는 SurfaceNet, Atlas, TransformerFusion 같은 직접 3D 특징 학습 방법이 연구되었고, 실외 자율주행에서는 MonoScene(단안), TPVFormer(멀티카메라, 희소 LiDAR supervision) 등 복셀 기반 장면 표현 연구가 이어졌습니다.
- **기존 한계점**:
  1. bounding box 기반 3D 검출은 임의의 형태나 무한한 클래스의 객체를 표현하기 어려움
  2. depth 기반 방법은 각 광선에서 가장 가까운 점만 예측하므로 가려진(occluded) 영역을 복원할 수 없음
  3. BEV 표현은 높이 방향 정보를 압축하여 3D 공간 정보를 손실함
  4. nuScenes 같은 데이터셋은 단일 프레임의 희소 LiDAR 포인트만 제공하여 밀집 occupancy 학습에 부적합하며, TPVFormer 등 희소 GT로 학습한 모델은 예측 결과도 희소함
- **이 논문의 접근 방식**: 2D-3D 공간 어텐션으로 BEV 대신 3D 볼륨 쿼리를 구성해 높이 정보 손실을 막고, 멀티프레임 LiDAR 스티칭과 Poisson Reconstruction으로 밀집 GT를 자동 생성하여 위 한계들을 동시에 해결합니다.
...
## 🛠️ Section 4: Dense Occupancy Ground Truth Generation
...
- **사용 데이터셋 / 벤치마크**: nuScenes(멀티카메라), SemanticKITTI(단안 카메라, 제로샷 평가)
...
**핵심 개념**
- **IoU (Intersection over Union)**: 예측과 정답이 겹치는 비율
...
- **주요 성과**: nuScenes에서 SSC mIoU 20.30으로 BEVFormer(16.75), TPVFormer(17.10) 대비 큰 폭 향상. 3D 장면 재구성에서도 TransformerFusion 대비 CD/F-score 모두 개선. 희소 GT 대신 밀집 GT로 학습하면 SSC mIoU가 12.17 → 20.30으로 대폭 향상되어, 밀집 GT 생성 파이프라인 자체의 효과가 입증됨.

## 💡 결론 및 시사점 (Conclusion & Insights)
SurroundOcc는 멀티카메라 이미지만으로 밀집하고 정확한 3D 시맨틱 점유를 예측하는 방법을 제시합니다. 2D-3D 공간 어텐션으로 BEV 대신 3D 볼륨 특징을 구성하고, 멀티프레임 LiDAR 스티칭 + Poisson Reconstruction으로 비용 없이 밀집 GT를 자동 생성하여, nuScenes와 SemanticKITTI 벤치마크에서 기존 방법 대비 큰 폭의 성능 향상을 달성했습니다.
...
- **한계점 및 아쉬운 점**: 단일 프레임 입력만 처리하므로 시간적 연속성(occupancy flow)이 없어 motion prediction에 직접 활용하기 어렵습니다. 또한 BEVFormer 대비 latency/memory가 다소 증가하여 실시간성과 정확도 사이의 트레이드오프가 존재하며, GT 생성 파이프라인이 멀티프레임 LiDAR 데이터에 의존하므로 LiDAR가 없는 순수 카메라 데이터셋에는 그대로 적용하기 어렵습니다.
